Example Usage
~~~~~~~~~~~~~
Example usage of ``budget-line`` in context of ``total-budget`` element.

| An example ``ref`` attribute is included.

| Multiple budget lines are expressed by repeating the ``budget-line`` element, within the ``total-budget``.
