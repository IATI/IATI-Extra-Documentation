Example Usage
~~~~~~~~~~~~~
| Example usage of ``language`` of a ``document-link`` in a ``result`` element.
...
2.03
^^^^
The optional ``language`` element of a ``document-link`` in a ``result`` element was `added <https://discuss.iatistandard.org/t/add-document-link-to-results-indicator-included-2-03/895>`__.
